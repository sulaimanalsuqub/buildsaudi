            ...
            {!brands || brands.length === 0 ? (
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-[#1D3F1F]/[0.06]">
                  {brands.map((brand) => {
                    const vendorBrands = brand.vendor_brands as {
                      vendor_id: string;
                      vendors: { establishment_name: string } | null;
                    }[];
                    const vendorNames = vendorBrands
                      .map((vb) => vb.vendors?.establishment_name)
                      .filter(Boolean) as string[];

                    return (
                      <tr key={brand.id} className="hover:bg-[#F4F3EB]/30">
                        <td className="px-4 py-3 font-semibold text-[#1D3F1F]">
                          {brand.name}
                        </td>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex flex-wrap gap-1">
                            {vendorNames.slice(0, 3).map((name) => (
                              <span key={name} className="rounded-full border border-[#1D3F1F]/10 bg-[#F4F3EB] px-2 py-0.5 text-xs text-[#1D3F1F]/70">
                                {name}
                              </span>
                            ))}
                            {vendorNames.length > 3 && (
