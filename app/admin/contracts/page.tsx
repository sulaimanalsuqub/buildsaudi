...
export default async function AdminContractsPage() {
  ...
  const signedVendorIds = new Set(signatures.filter((s) => s.signed_at).map((s) => s.vendor_id));
  const sentVendorIds = new Set(signatures.map((s) => s.vendor_id));
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="lg:col-span-1 space-y-4">
            ...
            <div className="rounded-[16px] border border-[#1D3F1F]/10 bg-white p-5">
              ...
              <div className="mt-4 grid grid-cols-3 gap-2 text-center">
                <div className="rounded-lg bg-[#F4F3EB] p-2">
                  <p className="text-lg font-bold text-[#1D3F1F]">{vendors.length}</p>
          ...
          {contract && unsentVendors.length > 0 && (
            <div className="rounded-[16px] border border-[#1D3F1F]/10 bg-white p-5">
              ...
              <SendContractButton contractId={contract.id} vendors={unsentVendors} label="إرسال للكل" />
            </div>
          )}
        </div>
        ...
        <div className="lg:col-span-2">
          <div className="rounded-[16px] border border-[#1D3F1F]/10 bg-white overflow-hidden">
            ...
            {signatures.length === 0 ? (
              <div className="py-12 text-center text-sm text-[#1D3F1F]/40">لا توجد توقيعات بعد</div>
            ) : (
                  ...
                  {signatures.map((sig) => {
                    ...
                    return (
                      <tr key={sig.id} className="hover:bg-[#F4F3EB]/30">
                        ...
                        <td className="px-4 py-3 text-[#1D3F1F]/60 text-xs" dir="ltr">{vendor?.email ?? "—"}</td>
                        <td className="px-4 py-3">
                          {sig.signed_at
                            ? <span className="rounded-full bg-emerald-100 px-2.5 py-1 text-xs font-semibold text-emerald-700">✓ وقّع</span>
                            : <span className="rounded-full bg-orange-100 px-2.5 py-1 text-xs font-semibold text-orange-700">بانتظار التوقيع</span>}
                        </td>
                        <td className="px-4 py-3 text-[#1D3F1F]/50 text-xs" dir="ltr">
                          {sig.signed_at ? new Date(sig.signed_at).toLocaleDateString("ar-SA") : "—"}
                        </td>
                        <td className="px-4 py-3">
                          {!sig.signed_at && (
                            <SendContractButton
