...
// href always points at the Arabic page — it's the only BANI route that exists
// (app/ar/register/bani), and BaniChat has its own in-panel language switcher, so an English
// visitor still ends up able to chat in English once inside.
export function BaniEntry({ isRtl = false }: BaniEntryProps) {
  const t = {
    title: textByLang(isRtl, "Register faster with BANI", "سجّل أسرع مع باني"),
    body: textByLang(
      isRtl,
